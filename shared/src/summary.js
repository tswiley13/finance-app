...
export function buildFinancialSummary({
  ...
  const genStr = generatedAt ? fmtDate(typeof generatedAt === "string" ? generatedAt.slice(0, 10) : toISO(generatedAt)) : "";
  ...
  md.push("## Debts");
  if (openDebts.length) {
    md.push("| Debt | Balance | APR | Min payment |");
    md.push("|---|---|---|---|");
    openDebts
      .slice()
      .sort((a, b) => (b.balance || 0) - (a.balance || 0))
      .forEach((d) => md.push(`| ${d.name} | ${fmtMoney(d.balance)} | ${pct(d.interest_rate) || "—"} | ${fmtMoney(d.minimum_payment)} |`));
  ...
  openDebts.forEach((d) =>
    csvRows.push(["Debt", d.name, "", `${pct(d.interest_rate)} APR, min ${num(d.minimum_payment)}`, num(d.balance)]));
  rows.forEach((r) => {
    const label = `${fmtShort(r.period.start_date)}-${fmtShort(r.period.end_date)}${r.isCurrent || r.isCurrentPeriod ? " (current)" : ""}`;
