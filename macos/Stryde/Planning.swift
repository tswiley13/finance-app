...
// MARK: - Monthly Overview (full typical month, before anything's marked paid)
...
struct MonthlyOverviewView: View {
    ...
    var body: some View {
        ...
        let income = store.income
            ...
            .sorted { ($0.fixedAmount ?? 0) * Finance.incMult($0.frequency) > ($1.fixedAmount ?? 0) * Finance.incMult($1.frequency) }
        let mIncome = Finance.monthlyIncome(store.income)
        let mBills = Finance.monthlyBills(store.bills)

        Page(title: "Monthly Overview",
             subtitle: "A full, typical month — every recurring bill and income, before anything's marked paid.") { w in
            HStack(spacing: 12) {
                StatTile(label: "Monthly Income", value: mIncome, accent: .sGood)
                StatTile(label: "Monthly Bills", value: mBills, accent: .sWarn)
                StatTile(label: "Left Over / mo", value: mIncome - mBills, accent: mIncome - mBills < 0 ? .sBad : .sAccent)
            }

            let billsPanel = Panel(title: "Bills (monthly)", count: bills.count) {
                if bills.isEmpty { EmptyRow(text: "No recurring bills") }
                else {
                    ForEach(bills) { b in
                        Row(name: b.name,
                            sub: (b.category ?? "").isEmpty ? Finance.freqLabel(b.frequency) : "\(b.category!) · \(Finance.freqLabel(b.frequency))",
                            amount: b.amount * Finance.billMult(b.frequency), amountColor: .sInk)
                    }
                }
            }
            let incomePanel = Panel(title: "Income (monthly)", count: income.count) {
                if income.isEmpty { EmptyRow(text: "No income") }
                else {
                    ForEach(income) { i in
                        Row(name: i.name, sub: Finance.freqLabel(i.frequency),
                            amount: (i.fixedAmount ?? 0) * Finance.incMult(i.frequency), amountColor: .sGood)
                    }
                }
            }
